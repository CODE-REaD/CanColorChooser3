...
Component.extend({
    ...
    ViewModel: {
                ...
                function initFinalColors() {
                    ...
                    for (let c = 0; c < this.finalCols; c++) {
                        for (let r = 0; r < this.finalCols; r++) {
                            ...
                            aColorMap = new DefineMap({
                                red: red,
                                blu: blu,
                                grn: grn
                            });
                            colors.push(aColorMap);
                        }
                    }
                }
                initFinalColors.call(this, {}, this.finalCols);

                listenTo("baseOrSuggestedColor", function (ev, color) {
                    // console.log(`listener responded to baseOrSuggestedColor`);
                    // todo: D.R.Y.: cf. initFinalColors()
                    let red;
                    let grn;
                    let blu;
                    let xOffset;
                    ...
                    let ySpread = Math.round(154 / this.finalCols);
                    let periphCols = Math.trunc(this.finalCols / 2);
                    for (let elNum = 0, len = colors.length; elNum < len; elNum++) {
                        // colors.forEach(function (aColor) { // See https://coderwall.com/p/kvzbpa/don-t-use-array-foreach-use-for-instead
                        xOffset = periphCols - elNum % this.finalCols;    // How far am I from grid center?
                        yOffset = periphCols - Math.trunc(elNum / this.finalCols);
                        // Purely empirical way of getting a useful range:
                        red = this.baseOrSuggestedColor.red + xOffset * xSpread + yOffset * ySpread;
                        grn = this.baseOrSuggestedColor.grn + xOffset * xSpread + yOffset * ySpread;
                        blu = this.baseOrSuggestedColor.blu + xOffset * xSpread + yOffset * ySpread;
                        ...
                        grn = grn > 255 ? 255 : grn < 1 ? 0 : grn;
                        blu = blu > 255 ? 255 : blu < 1 ? 0 : blu;
                        colors[elNum].red = red;
                        colors[elNum].grn = grn;
                        colors[elNum].blu = blu;
                    }

                    if (doDemo) {
                        let randomCellNum;
                        do
                            randomCellNum = Math.trunc(Math.random() * colors.length); // For demo mode
                        while (this.suggestedFinalColor === colors[randomCellNum]);
                        this.suggestedFinalColor = colors[randomCellNum];
                        frameCount++;
                    }
                });
            }
        ...
        // HELPER METHODS
        colorStyle(color) {
            if (color.red + color.grn * 1.5 + color.blu * .5 > 400)
